...
#				LF.logFunc(logloc=logloc, line=logShortcut(msg = "ran postHold()", hs = hs))
				if resultAPI[3] != 200:
					...
					LF.prowl(pmsg=now + ": postHold() received a non-200 response.", short="non-200 response") # send to prowl
...
#				LF.logFunc(logloc = logloc, line = logShortcut(msg = "ran resumeProgram()", hs = hs))
				if resultAPI[3] != 200:
					resultCode = "BAD"
					now = time.strftime("%H:%M:%S", time.localtime())
					LF.prowl(pmsg=now + ": resumeProgram() received a non-200 response.", short="non-200 response") # send to prowl
